...
def main():
    ...
    while True:
        ...
        while move not in pos.gen_moves():
            ...
            user_interface.show_char("*")
            user_move = user_interface.wait_on_move()
            print("Your move: ", user_move)
            match = re.match('([a-h][1-8])'*2, user_move) #input('Your move: '))
            if match:
                move = parse(match.group(1)), parse(match.group(2))
            else:
                # Inform the user when invalid input (e.g. "help") is entered
                print("Please enter a move like g8f6")
                user_interface.show_char("X")
        pos = pos.move(move)
        ...
        move_text = render(119-move[0]) + render(119-move[1])
        print("My move:", move_text)
        user_interface.show_move(move_text)
        pos = pos.move(move)
